...
已提供跨平台启动脚本，自动完成「建虚拟环境 → 装后端依赖 → 检查 `.env` → 装前端依赖 → 同时拉起前后端」。
...
首次运行会在复制出 `backend/.env` 后停下，提示你填入模型 API Key（`LLM_API_KEY`）；填好后**再运行一次**脚本即可。启动后浏览器打开 http://localhost:3000 使用。

> 前置：本机需已安装 **Python 3.10+** 与 **Node.js 18+**。
> 若偏好手动分步启动，见下方「快速开始」。
